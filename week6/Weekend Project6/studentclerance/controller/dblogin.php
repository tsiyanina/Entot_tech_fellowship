    ...
    public function userlogin($user,$pass){
        ...
        $stm->bind_param("ss",$newpas,$user);
         if($stm->execute()){
             ...
             public function request($date,$reason,$type){
                $stm= $this->conn->prepare("insert into request(Last_class_date,message,type)values(?,?,?);");
                $stm->bind_param("sss",$date,$reason,$type);
                if($stm->execute())
                    {
                    return 1;
                }else{
